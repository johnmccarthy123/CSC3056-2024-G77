...
public class SimpleBankingApp {
	...
	public static void printAllUsers() {
	    ...
	    for (int i = 0; i < users.size(); i++) {
	        ...
	        System.out.println(String.format("%-25s | %-15s | %-15s | %-15s | %-15s",
	                user.getUsername(), user.getPassword(), user.getFirst_name(), user.getLast_name(), user.getMobile_number()));
	    }
	    System.out.println();
	}
	...
	public static void printAllAccounts() {
	    ...
	    System.out.println(String.format("%-20s | %-25s | %-10s | %-15s | %-15s",
	            "Account #", "username_of_account_holder", "type", "opening_date", "Balance"));
	    System.out.println(String.join("", Collections.nCopies(95, "-"))); // Creates a dividing line of 95 dashes
	    
	    for (Account account : accounts) {
	        String formattedDate = dateFormat.format(account.getAccountOpeningDate());
	        String formattedBalance = balanceFormat.format(getBalance(account.getAccountNumber()));
	        System.out.println(String.format("%-20s | %-25s | %-10s | %-15s | $%14s",
	                account.getAccountNumber(), account.getUsernameOfAccountHolder(),
	                account.getAccountType(), formattedDate, formattedBalance));
	    }
	...
	public static double getBalance(String account_number) {
	    ...
	    for (Transaction transaction : transactions) {
	        if (transaction.getAccountNumber().equals(account_number)) {
	            balance += transaction.getTransactionAmount(); // Use the correct getter method
	        }
	    }
	    return balance;
	}
