...
public class SimpleBankingAppTest {
	...
	public static void testWithdrawals() {
	    // 1-Setup phase
	    String accountNumber = "5495-1234"; // Account number to test
	    double withdrawalAmount = 30.00; // The amount to withdraw
	    double balanceBefore = SimpleBankingApp.getBalance(accountNumber);

	    // 2-Exercise phase
	    // Perform a withdrawal by adding a transaction with a negative amount
	    ...
	    // 3-verify
	    // Verify that the balance after withdrawal is less than the original balance by the withdrawal amount
	    assert balanceBefore - withdrawalAmount == balanceAfter : "Withdrawal did not decrease the balance correctly";
	    if (balanceBefore - withdrawalAmount == balanceAfter)
	        ...
	        System.out.println(TestUtils.TEXT_COLOR_RED + "testAddTransactionWithdrawals: TC1 FAILED: Expected balance after withdrawal did not match actual.");
	    ...
	    // 4-tear-down
	    // Revert the withdrawal to restore the original account state
	    SimpleBankingApp.addTransaction(accountNumber, withdrawalAmount);
	}
